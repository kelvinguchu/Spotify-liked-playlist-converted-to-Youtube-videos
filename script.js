...
async function searchYoutube(songName, artistName) {
  ...
  const query = encodeURIComponent(
    `${songName} ${artistName} official music video`
  );
  const response = await fetch(
    `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&videoEmbeddable=true&regionCode=US&type=video&q=${query}&key=${apiKey}`
  );
  const data = await response.json();
...
async function handleAuthResponse() {
  ...
  try {
    ...
    const youtubeLinks = await Promise.all(
      songs.map((song) => searchYoutube(song.name, song.artists[0].name))
    );
...
function applyInitialTheme() {
  ...
  const darkModeButtonAuthorized = document.getElementById(
    "darkModeButtonAuthorized"
  );
...
searchField.addEventListener("input", async function () {
  ...
  const youtubeLinks = await Promise.all(
    filteredSongs.map((song) => searchYoutube(song.name, song.artists[0].name))
  );
...
document
  .getElementById("darkModeButtonLogin")
  .addEventListener("click", toggleDarkMode);
document
  .getElementById("darkModeButtonAuthorized")
  .addEventListener("click", toggleDarkMode);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const tokenExpiryTimestamp = sessionStorage.getItem(
  ...
  if (
    accessToken &&
    (!tokenExpiryTimestamp || currentTimestamp > tokenExpiryTimestamp)
  ) {
